...
function pinCircle(cx, cy, r, color){
  return `<circle cx="${cx}" cy="${cy}" r="${r}" fill="none" stroke="${color}" stroke-width="${(r*0.55).toFixed(1)}"/>` +
         `<circle cx="${cx}" cy="${cy}" r="${(r*0.3).toFixed(1)}" fill="${color}"/>`;
}
const PIN = {
...
function stick(cx, cy, len, color){
  const x = cx - 3.5, y = cy - len/2;
  return `<rect x="${x}" y="${y}" width="7" height="${len}" rx="3.5" fill="${color}"/>` +
         `<line x1="${x}" y1="${(cy-len/6).toFixed(1)}" x2="${x+7}" y2="${(cy-len/6).toFixed(1)}" stroke="#fffdf4" stroke-width="1.7"/>` +
         `<line x1="${x}" y1="${(cy+len/6).toFixed(1)}" x2="${x+7}" y2="${(cy+len/6).toFixed(1)}" stroke="#fffdf4" stroke-width="1.7"/>`;
}
const SOU = {
...
function face(id, red){
  if (id < 9){
    ...
    return `<text x="30" y="24" text-anchor="middle" dominant-baseline="central" font-family="${SERIF}" font-weight="800" font-size="31" fill="${numColor}">${KANJI_NUM[id]}</text>` +
           `<text x="30" y="62" text-anchor="middle" dominant-baseline="central" font-family="${SERIF}" font-weight="800" font-size="31" fill="${RED}">萬</text>`;
  }
  if (id < 18){
    const n = id - 9 + 1;
    if (n === 1){
...
function scoreWin(o){
  ...
  for (const dec of decomposeWin(c)){
    ...
    if (menzen && !pairYakuhai && dec.sets.every(s=>s.run) && o.winTile != null && o.winTile !== dec.pair){
...
function winProbEst(sh, ukeCount, junme, waitsLeft){
  if (sh <= 0){
    ...
    if (w <= 1) base -= 0.24;        // 枯れかけ待ち
    else if (w <= 2) base -= 0.16;   // 残り2枚の愚形
    else if (w <= 4) base -= 0.09;   // 愚形
...
const out = [];
// 副露込み向聴数: チー1つ + 手牌10枚テンパイ
out.push(t("副露1・聴牌", shanten(parseHand("123m456p78s99s"), 1), 0));
out.push(t("副露1・和了", shanten(parseHand("123m456p789s99s"), 1), -1));
out.push(t("副露2・1向聴", shanten(parseHand("123m45p29s"), 2), 1));
// 喰いタンヤオ: チー(234s) + 234m567m567p22p 和了
let r = scoreWin({ hand14: parseHand("234m567m567p22p"), melds:[{t:19,run:true}], winTile: 15, tsumo: false, riichi:false, ippatsu:false, seatWind:28, roundWind:27, doraInd:[], uraInd:null, dealer:false, kuitan:true });
out.push(t("喰い断1翻", r ? r.han : null, 1));
// 喰い断なしルール → 役なし
r = scoreWin({ hand14: parseHand("234m567m567p22p"), melds:[{t:19,run:true}], winTile: 15, tsumo: false, riichi:false, ippatsu:false, seatWind:28, roundWind:27, doraInd:[], uraInd:null, dealer:false, kuitan:false });
out.push(t("喰い断なし→役なし", r, null));
// 役牌ポン + 平和なし: 白ポン + 234m567p88s + 66s? 白pon {t:31,run:false}, concealed "234m567p66788s"=11: 234m,567p,678s,88s? win 8s
r = scoreWin({ hand14: parseHand("234m567p67888s"), melds:[{t:31,run:false}], winTile: 25, tsumo: true, riichi:false, ippatsu:false, seatWind:28, roundWind:27, doraInd:[], uraInd:null, dealer:false, kuitan:true });
out.push(t("白ポン(ツモ・門前ツモ無し)", r ? [r.han, r.yaku.some(y=>y[0]==="白"), r.yaku.some(y=>y[0]==="門前清自摸和")] : null, [1, true, false]));
// 対々和: 111zポン + 333p777s888s99m(11枚) ロン
r = scoreWin({ hand14: parseHand("333p777888s99m"), melds:[{t:27,run:false}], winTile: 8, tsumo: false, riichi:false, ippatsu:false, seatWind:27, roundWind:27, doraInd:[], uraInd:null, dealer:true, kuitan:true });
out.push(t("東ポン対々(ダブ東+対々=4翻)", r ? [r.han >= 4, r.yaku.some(y=>y[0]==="対々和")] : null, [true, true]));
// 混一色 副露 2翻に減少
r = scoreWin({ hand14: parseHand("111234567m77z"), melds:[{t:6,run:true}], winTile: 5, tsumo: false, riichi:false, ippatsu:false, seatWind:28, roundWind:29, doraInd:[], uraInd:null, dealer:false, kuitan:true });
out.push(t("鳴き混一=2翻", r ? r.yaku.some(y=>y[0]==="混一色"&&y[1]===2) : null, true));
// 門前は従来通り
r = scoreWin({ hand14: parseHand("234567m234p56799s"), winTile: 24, tsumo: true, riichi: false, ippatsu: false, seatWind: 28, roundWind: 27, doraInd: [], uraInd: null, dealer: false });
out.push(t("門前平和ツモ(回帰)", [r.han, r.fu], [2, 20]));
