...
const out = [];
// 白の暗槓（門前維持）+ ツモ: 門前ツモ1 + 白1 + カンドラ4(表示=中→ドラ白) = 6翻
let r = scoreWin({ hand14: parseHand("234m567p67888s"), melds:[{t:31,run:false,kan:true}], menzen:true,
  winTile: 23, tsumo: true, riichi:false, ippatsu:false, seatWind:28, roundWind:27, doraInd:[33], uraInd:null, dealer:false, kuitan:true });
out.push(t("暗槓白+ツモ+カンドラ4", r ? [r.han, r.yaku.some(y=>y[0]==="門前清自摸和"), r.yaku.some(y=>y[0]==="ドラ"&&y[1]===4)] : null, [6, true, true]));
// 暗槓ありでも立直可（menzen扱い）→ 立直+ツモ+白
r = scoreWin({ hand14: parseHand("234m567p67888s"), melds:[{t:31,run:false,kan:true}], menzen:true,
  winTile: 23, tsumo: true, riichi:true, ippatsu:false, seatWind:28, roundWind:27, doraInd:[], uraInd:null, dealer:false, kuitan:true });
out.push(t("暗槓+立直有効", r ? r.yaku.some(y=>y[0]==="立直") : null, true));
// 大明槓(明カン)は門前でない → ツモ役なし・白のみ
r = scoreWin({ hand14: parseHand("234m567p67888s"), melds:[{t:31,run:false,kan:true}], menzen:false,
  winTile: 23, tsumo: true, riichi:false, ippatsu:false, seatWind:28, roundWind:27, doraInd:[], uraInd:null, dealer:false, kuitan:true });
out.push(t("明カンは門前ツモ無し", r ? [r.han, r.yaku.some(y=>y[0]==="門前清自摸和")] : null, [1, false]));
// 暗槓で平和は不成立
r = scoreWin({ hand14: parseHand("234m567p56799s"), melds:[{t:0,run:false,kan:true}], menzen:true,
  winTile: 24, tsumo: true, riichi:false, ippatsu:false, seatWind:28, roundWind:27, doraInd:[], uraInd:null, dealer:false, kuitan:true });
out.push(t("暗槓あり=平和なし", r ? r.yaku.some(y=>y[0]==="平和") : "null_ok", false));
